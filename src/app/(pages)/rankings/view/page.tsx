      ...
      <div className="space-y-6">
        {(!rankedTiers || rankedTiers.length === 0) && (
          <div className="text-gray-500 italic">
            No ranked items yet. Check back later!
          </div>
        )}
            ...
            <div className="space-y-3">
              ...
              {rankedTiers.map((tierItems, index) => (
                <div key={index} className="flex items-start">
                  <div className="w-8 h-8 flex-shrink-0 flex items-center justify-center bg-gray-200 rounded-full font-bold text-gray-600 mr-4">
                    {index + 1}
                  </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {unrankedItems.map((item) => (
                <span
                  key={item}
                  className="bg-gray-100 text-gray-700 px-3 py-1 text-sm font-medium rounded-full"
                >
                  {item}
                </span>
              ))}
...
const SummaryView: React.FC = () => {
  ...
  const handleQuizFriend = async () => {
    ...
    try {
      ...
      const updateOwnEntryResponse = await updateRankingWithConfirmation(
        id1,
        ranking1,
      ...
      if (!updateOwnEntryResponse || !updateOwnEntryResponse.ok) {
        throw new Error(
          `Failed to update own entry with metadata pointing to new entry for other friend to fill out. Response: ${updateOwnEntryResponse}`,
        );
      }
    } catch (error) {
  ...
  const handleGetSharedLink = () => {
    ...
    if (!friendRankingId)
      throw new Error(
        "No friend ranking ID found even though we should have already generated a shared link; cannot reconstruct shared link",
      );
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex justify-center">Loading view...</div>
    );
  }
  ...
  if (!ranking1) {
    return (
      <div className="min-h-screen flex justify-center">
        Could not load ranking data. The link may be invalid.
      </div>
    );
  }
